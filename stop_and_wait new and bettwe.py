...
class StopAndWait:
        ...
        while True:
            ...
            if in_xfr:
                ...
                if data[:9] == B"///END\\\\\\":#maby change for header
                    ...
                    checksum = SumOfBits(data)
                    checksumInBytes = bytes(str(checksum),'utf-8')
                    
                    #makes checksum 5 digits
                    while len(str(checksum))<5:
                        Zero2front = bytes(str(0),'utf-8')
                        checksumInBytes = checksumInBytes + Zero2front
                        
                    #gathers header from the UDP paylaod
                    recivedSequence = data[0]
                    recivedChecksum = data[1:6]
                    
                    #if message recived proporly
                    if recivedSequence == expectedSequence and recivedChecksum == checksumInBytes:
                        # just write the data...
                        ...
                        if expectedSequence == B"1":
                            expectedSequence = B"0"
                        else:
                            expectedSequence = B"1"
                ...
                if data[:5] == B'BEGIN':#mabye change for header
    ...
    def xfr(self, sock, payload, dest, mtu):
        ...
        for i,chunk in enumerate(payload):
            ...
            while len(str(checksum))<5:
                Zero2front = bytes(str(0),'utf-8')
                clientChecksumInBytes = clientChecksumInBytes + Zero2front
            ...
            while tries < self.retries:
                ...
                stuff, crap = chunk
                newChunk = self.chunk(sequence + clientChecksumInBytes + stuff,1)
    ...
    def SumOfBits(self, listofBits):
        ...
        temp = 0
        for i in len(listofBits):
            temp = temp + listofBits[i]
        return temp
